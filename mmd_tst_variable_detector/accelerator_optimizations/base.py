...
class BaseTaskDispatcher(ABC):
    ...
    def __init__(
        ...
        if worker_fn is not None:
            ...
            self.worker_fn = worker_execution_routine
        # end if
        ...
        for on_job_batch in seq_batch:
            ...
            if self.resume_checkpoint_saver is not None:
                for sub_learner_result in batch_results:
                    if hasattr(self.resume_checkpoint_saver, "save_checkpoint"):
    ...
    def _log_post_process(
        self, seq_results_one_batch: ty.List[ty.Any]
    ) -> None:
        ...
        for res in seq_results_one_batch:
            if hasattr(res, "get_job_id_string"):
                run_name = res.get_job_id_string()
            elif hasattr(res, "get_key_id"):
                run_name = res.get_key_id()
            else:
                run_name = str(id(res))
            # end if
