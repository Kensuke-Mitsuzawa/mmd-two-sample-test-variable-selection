...
class RamBackendStaticDataset(BaseRamBackendDataset, BaseStaticDataset, BaseSampleBasedDataset):
    def __init__(self,
                 x: torch.Tensor,
                 y: torch.Tensor,
    ...
    def get_dimension_data_space(self) -> ty.Tuple[int,...]:
        _dim_x = self.get_dimension_flattened()
        return tuple([_dim_x])
    ...
    def get_dimension_flattened(self) -> int:
        ...
        assert __x == __y, f'Inconsistent dimension. X={__x} Y={__y}'
    ...
    def get_subsample_dataset(self, 
                              ...
                              ) -> ty.Tuple[ty.List[int], "SimpleDataset"]:
    ...
    def get_bootstrap_dataset(self, seed: ty.Optional[int] = None) -> ty.Tuple[ty.Tuple[ty.List[int], ty.List[int]], "SimpleDataset"]:
        if seed is None:
    ...
    def get_selected_variables_dataset(self, selected_variables: ty.List[int]) -> "SimpleDataset":
        ...
        dim_x = self.get_dimension_flattened()

        x = torch.zeros(len(seq_tuple_tensor_xy), dim_x)
        y = torch.zeros(len(seq_tuple_tensor_xy), dim_x)
    ...
    def split_train_and_test(self, 
        ...
        seq_train_id, train_dataset = self.get_subsample_dataset(
            n_samples=n_sample_train,
            random_seed=random_seed)
        
        set_test_sample_id = set(seq_sample_id) - set(seq_train_id)
        seq_test_id, test_dataset = self.get_subsample_dataset(
            sample_ids=list(set_test_sample_id))        
...
SimpleDataset = RamBackendStaticDataset
