...
from mmd_tst_variable_detector.datasets import FileBackendSensorSampleBasedDataset
...
def test_dataset_matrix():
    ...
    dataset_first = FileBackendSensorSampleBasedDataset(sample_set_first)
    ...
    assert t_xy[0].shape == (20, 20)
    assert t_xy[1].shape == (20, 20)
    ...
    dataset_second = FileBackendSensorSampleBasedDataset(sample_set_second)

    dataset_obj_merged = dataset_first.merge_new_dataset(dataset_second)
    assert len(dataset_obj_merged) == 20
    
    logger.warning('IMPORTANT: I skip `selected_variables` test. Not ready yet. This dataset is not ready to use!!!!')    
