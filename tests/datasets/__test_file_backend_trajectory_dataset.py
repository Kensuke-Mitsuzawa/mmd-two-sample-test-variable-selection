...
# from mmd_tst_variable_detector.datasets import FileBackendTrajectoryTimeSlicingDataset


# def generate_trajectory_array(path_file_root: Path) -> ty.List[Path]:
#     """Generating 3-D array of (|A|, |T|, C). I split all T elements into separated files. A separated file has 2D array of (|A|, C). 
#     """
#     seq_file_parent_dir = []
...
#     __x = np.random.normal(size=(size_a, size_t, size_c))
#     __y = np.random.normal(size=(size_a, size_t, size_c))    
...
#     seq_timestlice_files_first = generate_trajectory_array(path_root_data_dir_first)
    
#     time_slicing_size = 10
#     time_start_current = 0
    
#     while (time_start_current + time_slicing_size) < 100:
#         __dataset_part = FileBackendTrajectoryTimeSlicingDataset(
...
#     # merging test
#     dataset_first = FileBackendTrajectoryTimeSlicingDataset(
#             path_pair_parent_dir=seq_timestlice_files_first,
#             time_slice_from=0,
#             time_slice_to=99)
...
#     seq_timestlice_files_second = generate_trajectory_array(path_root_data_dir_second)

#     dataset_second = FileBackendTrajectoryTimeSlicingDataset(
