...
from mmd_tst_variable_detector.datasets import RamBackendStaticDataset
...
def test_x_y_unequal_samples():
    ...
    y_train_plus = torch.concatenate([t_xy_train[1], t_xy_train[1]])
    # dataset generationg
    dataset_train = RamBackendStaticDataset(t_xy_train[0], y_train_plus)
    dataset_dev = RamBackendStaticDataset(t_xy_dev[0], t_xy_dev[1])
    dataset_test = RamBackendStaticDataset(t_xy_test[0], t_xy_test[1])
